...
function PickableShapes:sv_n_tunnel(data)
    self.network:sendToClients("cl_n_getItem", { player = data.player })
end
...
function PickableShapes:cl_n_getItem(data)
    local slot = sm.localPlayer.getSelectedHotbarSlot()
    local container = isSurvival and data.player:getInventory() or data.player:getHotbar()
